...
$invoice_number=mt_rand();  //mt_rand() is taken in method taken by w3 schools.
$count_products=mysqli_num_rows($result_cart_price);
$product_values = [];  // Initialize the array to store product prices.
$status='pending';
while($row_price=mysqli_fetch_array($result_cart_price)){
    ...
    while($row_product_price=mysqli_fetch_array( $run_price)){
        $product_price=array($row_product_price['product_price']);  //each time we have to store the product price so we are adding array here.
        $product_values=array_sum($product_price);  //here I sum that all values.
...
else{//whatever the quantity the user is genrated that is going to be assigned.
    $quantity=$quantity;
